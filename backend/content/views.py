...
def import_template(request):
	...
	# ── Hidden lookups sheet (avoids 255-char formula limit for long lists) ──
	...
	fill_req = PatternFill("solid", fgColor="D6EAF8")   # blue-ish — required
	fill_opt = PatternFill("solid", fgColor="EAF7EC")   # green-ish — optional
	...
	# domain_1…domain_5 — one dropdown per column (H–L)
	domain_formula = f"Lookups!$E$1:$E${len(domain_names)}" if domain_names else '"(no domains)"'
	domain_cols = ["H", "I", "J", "K", "L"]
	domain_dvs = []
	for col_letter in domain_cols:
		dv = DataValidation(
			type="list",
...
def import_confirm(request):
	...
	Body: {"rows": [...valid row objects, each with a "status" field...]}
	"""
	from content.ingestion.importer import ImportService  # noqa: PLC0415
