...
<template>
  ...
  <main>
    <div class="detail-wrap">
      ...
      <p v-if="loading" class="info-message">Loading…</p>
      <p v-else-if="error && !question" class="error-message">{{ error }}</p>
      ...
      <div v-if="question && !loading" class="detail-card">
        ...
        <template v-else>
          <div class="form-group">
            <label>Title</label>
            <input v-model="questionDraft.title" type="text" class="text-input" />
          </div>
          <div class="form-group">
            <label>Body</label>
            <textarea v-model="questionDraft.body" rows="6" class="text-area"></textarea>
          </div>
      ...
      <section v-if="question" class="answers-section">
        ...
        <p v-if="answerError" class="error-message">{{ answerError }}</p>
        ...
        <div class="answer-list">
          <div v-for="answer in answers" :key="answer.id" class="answer-card">
            ...
            <template v-else>
              <textarea v-model="answerEditDraft" rows="4" class="text-area"></textarea>
              <div class="answer-footer">
                <span></span>
        ...
        <div class="answer-compose">
          ...
          <p v-else class="info-message">
            <RouterLink :to="{ name: 'Auth', query: { redirect: route.fullPath } }">Log in</RouterLink>
            to post an answer.
          </p>
        </div>
      </section>
