...
- `GET/PATCH/DELETE /api/users/{user_id}`: admin-only read, update, or soft-delete a user.
- Organization endpoints: attach or remove a user-organization link for an admin or the authenticated user themself. Non-admin users can only link as `member`.
- `GET /dashboard`: return dashboard stats, recent favorites, and recent matches for an admin or the authenticated user themself.
- Needs endpoints: list, create, update, and delete user needs for an admin or the authenticated user themself.
- Favorites endpoints: list, add, and remove saved offers for an admin or the authenticated user themself.
- Matching hits endpoints: list recommendations and update match status for an admin or the authenticated user themself.
...
- Requests use path-based user scoping: every user resource is nested under `/api/users/{user_id}`. Authenticated self-service dashboard resources also accept `/api/users/me/...`.
